...
public class Badminton {
    ...
    public Badminton() {
        ...
        create_badminton_slots();
    }

    private static void create_badminton_slots() {
        for (int i = 1; i <= 10; i++) {
            String time = " " + (9 + i) + ":00-" + (10 + i) + ":00";
            available_slots.put(i, time);
        }
    }


    public static void book_Badminton_slot() {
        Scanner scanner = new Scanner(System.in);
        if (!available_slots.isEmpty()) {
            System.out.print("Enter your name: ");
            String name = scanner.next();
            System.out.println("These are the available slots for 'Badminton'");
            while (true) {
